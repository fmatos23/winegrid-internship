...
#if defined(CONFIG_WIFI)
/*
 * This Zephyr version has no persistent Wi-Fi credentials subsystem, so the
 * "wifi connect" shell command has to be retyped after every reset. Baking
 * SSID/PSK into the firmware here lets the board reconnect on its own at
 * boot instead - a PoC convenience, not a secure secret-storage mechanism
 * (see notas_estagio.md for the trade-off).
 */
...
int main(void)
{
	...
	LOG_INF(APP_BANNER " v2-ota-test");
...
#if defined(CONFIG_WIFI)
		/* Plain Wi-Fi STA has no built-in connectivity implementation, so
		 * conn_mgr_if_connect() below returns an error and skips waiting
		 * (by design, for interfaces that don't require connectivity).
		 * Issue the Wi-Fi connect ourselves so there's something to wait
		 * for instead of racing the LwM2M registration against a network
		 * that isn't up yet.
		 */
